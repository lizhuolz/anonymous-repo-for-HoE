...
from model_merging_methods.task_vector import TaskVector,TaskVectorFFT,TaskVectorBREAD,TaskVectorTWIN
from utils_file.utils import get_param_names_to_merge, get_modules_to_merge
...
class MergingMethod:
    ...
    def fourier_merging(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, scaling_coefficient: float = 1.0 ,high_ratio_for_fourier:float =0.6):
        """
        task arithmetic method
        :param merged_model: nn.Module, the merged model
        :param models_to_merge: list, individual models that need to be merged
        ...
        models_to_merge_task_vectors = [TaskVectorFFT(pretrained_model=merged_model, finetuned_model=model_to_merge, exclude_param_names_regex=exclude_param_names_regex,high_ratio_for_fourier=high_ratio_for_fourier) for model_to_merge in models_to_merge]
        # models_to_merge_task_vectors = self.align_each_layer(models_to_merge_task_vectors)
        # iterate each individual model that needs to be merged
        with torch.no_grad():
            # sum up the task vectors
            merged_task_vector = models_to_merge_task_vectors[0] + models_to_merge_task_vectors[1]
    ...
    def collect_each_dataset_layer_mean(self,task_vectors,datasets=None):
        dataset_layer_mean=[]
        for task_vector in task_vectors:
            cur_layer_mean=[]
            cur_vector=task_vector.task_vector_param_dict
    ...
    def align_each_layer(self,task_vectors):
        dataset_layer_mean=self.collect_each_dataset_layer_mean(task_vectors)
        ...
        for task_vector,res in zip(task_vectors,result):
            for key,ratio in zip(task_vector.task_vector_param_dict.keys(),res):
                # if ('mlp' in key or 'attn' in key):
                    task_vector.task_vector_param_dict[key]*=ratio
            new_task_vectors.append(task_vector)
        return new_task_vectors
    
    def bread_merging(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, scaling_coefficient: float = 1.0,top_k_keep:float=0.1,top_k_remove:float=0.01):
        """
        task arithmetic method
        :param merged_model: nn.Module, the merged model
        :param models_to_merge: list, individual models that need to be merged
        ...
        models_to_merge_task_vectors = [TaskVectorBREAD(pretrained_model=merged_model, finetuned_model=model_to_merge, exclude_param_names_regex=exclude_param_names_regex,top_k_keep=top_k_keep,top_k_remove=top_k_remove) for model_to_merge in models_to_merge]
    ...
    def free_merging(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, scaling_coefficient: float = 1.0 ,high_ratio_for_fourier:float =0.6,top_k_keep=0.02,top_k_remove=0.00):
        """
        task arithmetic method
        :param merged_model: nn.Module, the merged model
        :param models_to_merge: list, individual models that need to be merged
        ...
        models_to_merge_task_vectors = [TaskVectorFFT(pretrained_model=merged_model, finetuned_model=model_to_merge, exclude_param_names_regex=exclude_param_names_regex,high_ratio_for_fourier=high_ratio_for_fourier) for model_to_merge in models_to_merge]
        private_models= [TaskVectorBREAD(pretrained_model=merged_model, finetuned_model=model_to_merge, exclude_param_names_regex=exclude_param_names_regex,top_k_keep=top_k_keep,top_k_remove=top_k_remove) for model_to_merge in models_to_merge]
        # models_to_merge_task_vectors = self.align_each_layer(models_to_merge_task_vectors)
        # iterate each individual model that needs to be merged
        with torch.no_grad():
            # sum up the task vectors
            ...
            # combine with parameters of the merged model based on scaling coefficient
            # merged_params = merged_task_vector.combine_with_pretrained_model(pretrained_model=merged_model, scaling_coefficient=scaling_coefficient)
        return merged_task_vector.task_vector_param_dict,private_models
    
    def twin_merging(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, scaling_coefficient: float = 1.0 ,high_ratio_for_fourier:float =0.6,top_k_keep=0.02,top_k_remove=0.00):
        """
        task arithmetic method
        :param merged_model: nn.Module, the merged model
        :param models_to_merge: list, individual models that need to be merged
        ...
        models_to_merge_task_vectors = [TaskVector(pretrained_model=merged_model, finetuned_model=model_to_merge, exclude_param_names_regex=exclude_param_names_regex) for model_to_merge in models_to_merge]
        private_models= [TaskVectorTWIN(pretrained_model=merged_model, finetuned_model=model_to_merge, exclude_param_names_regex=exclude_param_names_regex,top_k_keep=top_k_keep) for model_to_merge in models_to_merge]
    ...
    def merging_models(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, trainers: list = None, scaling_coefficient: float = 1.0,
                       ...
                       mask_apply_method: str = "average_merging", models_use_deepcopy: bool = False,top_k_keep:float=0.1,top_k_remove:float=0.01,high_ratio_for_fourier:float=0.6):
        """
        ...
        if self.merging_method_name == "average_merging":
            ...
            return Vector_unified, masks, rescales
        elif self.merging_method_name == 'fr_merging':
            merged_params = self.fourier_merging(merged_model=merged_model, models_to_merge=models_to_merge, exclude_param_names_regex=exclude_param_names_regex,
                                                 scaling_coefficient=scaling_coefficient,high_ratio_for_fourier=high_ratio_for_fourier)
        elif self.merging_method_name == 'bread_merging':
            merged_params = self.bread_merging(merged_model=merged_model, models_to_merge=models_to_merge, exclude_param_names_regex=exclude_param_names_regex,
                                                 scaling_coefficient=scaling_coefficient,top_k_keep=top_k_keep,top_k_remove=top_k_remove)
        elif self.merging_method_name == 'free_merging':
            merged_params,private_models = self.free_merging(merged_model=merged_model, models_to_merge=models_to_merge, exclude_param_names_regex=exclude_param_names_regex,
                                                 scaling_coefficient=scaling_coefficient,high_ratio_for_fourier=high_ratio_for_fourier,top_k_keep=top_k_keep,top_k_remove=top_k_remove)
            return merged_params,private_models
        elif self.merging_method_name == 'twin_merging':
            merged_params,private_models = self.twin_merging(merged_model=merged_model, models_to_merge=models_to_merge, exclude_param_names_regex=exclude_param_names_regex,
                                                 scaling_coefficient=scaling_coefficient,high_ratio_for_fourier=high_ratio_for_fourier,top_k_keep=top_k_keep)
            return merged_params,private_models
    ...
    def get_merged_model(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, trainers: list = None, scaling_coefficient: float = 1.0,
        ...
        merged_params = self.merging_models(merged_model=merged_model, models_to_merge=models_to_merge, exclude_param_names_regex=exclude_param_names_regex, trainers=trainers,
                                            ...
                                            mask_apply_method=mask_apply_method, models_use_deepcopy=models_use_deepcopy,top_k_keep=top_k_keep,top_k_remove=top_k_remove,high_ratio_for_fourier=high_ratio_for_fourier)
    ...
    def get_private_merged_model(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, top_k_keep,top_k_remove,high_ratio_for_fourier,models_use_deepcopy: bool = False):

        merged_params,private_models = self.merging_models(merged_model=merged_model, models_to_merge=models_to_merge,
                                                  exclude_param_names_regex=exclude_param_names_regex,
                                                  models_use_deepcopy=models_use_deepcopy,
    ...
    def get_twin_merged_model(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, top_k_keep,top_k_remove,high_ratio_for_fourier=0,models_use_deepcopy: bool = False):
