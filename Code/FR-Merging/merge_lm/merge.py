...
class MergingMethod:
    ...
    def fourier(
        ...
        for vector in task_vectors:
            cur_state_dict=vector.param_dict
            for key in cur_state_dict.keys():
                if 'weight' in key and cur_state_dict[key].ndim==2:
                    cur_state_dict[key]=self.find_low_high_fft(cur_state_dict[key],high_scale_ratio=0.6)
            new_task_vectors.append(param(cur_state_dict))
        scaling=0.4
        merged_param = base_model + scaling * sum(new_task_vectors)
        return merged_param
    
    def find_low_high_fft(self,tensor,high_scale_ratio):
        low_freq_spatial,high_freq_spatial,band_pass_spatial=self.cal_low_and_high_fft_for_tensor(tensor,high_scale_ratio)
        return low_freq_spatial+band_pass_spatial

    def cal_low_and_high_fft_for_tensor(self,weight,high_ratio_for_fourier):
        ...
        high_radius = max(H, W)*high_ratio_for_fourier
        center = (H // 2, W // 2)
        Y, X = torch.meshgrid(torch.arange(H), torch.arange(W))
        dist_from_center = torch.sqrt((X - center[1]) ** 2 + (Y - center[0]) ** 2)
        low_freq_mask = (dist_from_center <= low_radius).float().to(freq_domain.device)
        high_freq_mask = (dist_from_center > high_radius).float().to(freq_domain.device)

        band_pass_mask = ((dist_from_center >= low_radius) & (dist_from_center <= high_radius)).float().to(freq_domain.device)
