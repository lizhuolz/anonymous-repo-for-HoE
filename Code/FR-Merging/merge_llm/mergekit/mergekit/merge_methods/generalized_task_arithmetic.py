...
class GeneralizedTaskArithmeticMergeFourier(MergeMethod, BaseModel, frozen=True):
    consensus_method: Optional[ConsensusMethod]
    sparsification_method: Optional[SparsificationMethod]
    default_normalize: bool
    lambda_type: Optional[str] = "Constant"
...
class GTATaskFourier(Task[torch.Tensor]):
    method: GeneralizedTaskArithmeticMergeFourier
    tensors: GatherTensors
    base_model: ModelReference
    out_tensor_name: str
    tensor_parameters: ImmutableMap[ModelReference, Any]
        ...
        **_kwargs,
        ...
        for i,delta in enumerate(deltas):
            if delta.ndim==2:
                ...
                low,high,bank=cal_low_and_high_fft_for_tensor(delta.float().cpu(),high_ratio=0.9)
                deltas[i]=(low+bank).to(device)
        weights = torch.tensor(
            [tv["weight"] for tv in tvs], dtype=deltas.dtype, device=deltas.device
        )
        lambda_factor = tvs[0]["lambda"]
