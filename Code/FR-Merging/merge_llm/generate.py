...
def batch_response_gen_stop(input_texts):
    inputs = tokenizer.batch_encode_plus(input_texts, return_tensors="pt", padding=True, truncation=False)
    ...
    sampling_params = SamplingParams(temperature=0.1, top_p=1.0,max_tokens=512)
    with torch.no_grad():
        generation_output = model.generate(
            input_texts,
    ...
    for i in range(len(generation_output)):
        output = generation_output[i].outputs[0].text
        responses.append(output)
...
if __name__ == "__main__":
    ...
    if args.float32:
        model = LLM(model=model_name, trust_remote_code=False,tensor_parallel_size=4)
