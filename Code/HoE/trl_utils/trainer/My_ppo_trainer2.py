...
#   ref_model   ConditionedMOEModelWithValueHead ，     weights   router
#   ref_model     ，   My_batched_forward_pass_for_reference_model
...
class MyPPOTrainer2(BaseTrainer):
    """
    ...
    def generate(
        ...
        **generation_kwargs,
        ...
        """
        response_list = []
        for query, weight in zip(query_tensor, weights):
            self.accelerator.unwrap_model(self.model).set_weights(weight)
            with torch.no_grad():
                response = self.accelerator.unwrap_model(self.model).generate(
                    input_ids=query.unsqueeze(dim=0), **generation_kwargs
                )
            if not return_prompt and not self.is_encoder_decoder:
                response = response[:, query.shape[0] :]
    ...
    def _generate_batched(
        ...
        **generation_kwargs,
        ...
        for i in range(0, len(query_tensors), batch_size):
            ...
            batch_weights = weights[i:end_index].to(self.current_device)
            ...
            #self.accelerator.unwrap_model(model).set_weights(batch_weights)
            with torch.no_grad():
                generations = self.accelerator.unwrap_model(model).generate(weights=batch_weights, **padded_inputs, **generation_kwargs)
    ...
    def _step_safety_checker(
        ...
        weights: List[torch.FloatTensor],
        ...
        """
        for name, tensor_list in zip(["queries", "weights", "responses", "scores"], [queries, weights, responses, scores]):
        ...
        weights = [tensor.to(self.current_device) for tensor in weights]
        ...
        return queries, weights, responses, scores, masks
    ...
    def step(
        ...
        queries, weights, responses, scores, response_masks = self._step_safety_checker(
            bs, queries, weights, responses, scores, response_masks
        )
        ...
        model_inputs = self.prepare_model_inputs(queries, weights, responses)
        ...
        with torch.no_grad():
            ...
            #print("Hi! ref_policy forward...")
            #self.accelerator.unwrap_model(self.model).test_grad()

            ref_logprobs, ref_logits_or_none, _, _ = self.My_batched_forward_pass_for_reference_model(
                self.ref_model,
                queries,
                responses,
                model_inputs,
    ...
    def prepare_model_inputs(self, queries: torch.Tensor, weights: torch.Tensor, responses: torch.Tensor):
        if self.is_encoder_decoder:
        ...
        input_data["weights"] = torch.stack(weights).to(self.current_device)
        return input_data
    ...
    def My_batched_forward_pass(
        ...
        return (
            ...
            torch.cat(all_values)[:, :-1].to(self.current_device) ,
            torch.cat(all_masks)[:, :-1].to(self.current_device),
        )
    

    @PPODecorators.empty_device_cache()
    def My_batched_forward_pass_for_reference_model(
        self,
        #model: AutoModelForCausalLM,  #    nn.Module  ，       ref_model   AutoModelForCausalLM， class    weights
        model: nn.Module,
        queries: torch.Tensor,
        responses: torch.Tensor,
        model_inputs: dict,
        ...
        #  ref_model sft model,     weights    
        #model_inputs_wo_weights = {k:v  for k,v in model_inputs.items() if k!="weights"}

        for i in range(math.ceil(bs / fbs)):
            #input_kwargs = {key: value[i * fbs : (i + 1) * fbs] for key, value in model_inputs_wo_weights.items()}
            input_kwargs = {key: value[i * fbs : (i + 1) * fbs] for key, value in model_inputs.items()}
            query_batch = queries[i * fbs : (i + 1) * fbs]
            response_batch = responses[i * fbs : (i + 1) * fbs]
            if response_masks is not None:
            ...
            #logits = model.forward(**input_kwargs).logits
            #forward_output = self.accelerator.unwrap_model(model).forward(is_ref_model=is_ref_model, **input_kwargs)
            forward_output = self.accelerator.unwrap_model(model).forward(**input_kwargs)
            logits = forward_output["lm_logits"]
    ...
    def batched_forward_pass(
        ...
        for i in range(math.ceil(bs / fbs)):
            input_kwargs = {key: value[i * fbs : (i + 1) * fbs] for key, value in model_inputs.items()}
            query_batch = queries[i * fbs : (i + 1) * fbs]
            weight_batch = weights[i * fbs : (i + 1) * fbs]
            response_batch = responses[i * fbs : (i + 1) * fbs]
            if response_masks is not None:
                response_masks_batch = response_masks[i * fbs : (i + 1) * fbs]
            logits, _, values = model.forward(**input_kwargs)
    ...
    def train_minibatch(
        ...
        for name, param in self.model.named_parameters():
            if param.requires_grad == True and param.grad is None:
                print(f"The grad of {name} is none.")
                count = count+1
            if param.requires_grad == True and param.grad is not None and param.grad.sum() == 0.0:
                print(f"The grad of {name} is zero.")
                count = count+1
            if count ==5:
                break
        count = 0
        for param in self.optimizer.param_groups[0]["params"]:
            if param.grad is None:
                print("found:  grad is None")
                count = count+1
            if param.grad.sum() == 0.0:
                print("found:  grad is zero")
                count = count+1
            if count ==5:
                break
        print("no grad is None. no grad is zero")
        """
        self.optimizer.zero_grad()
        return train_stats
    ...
    def Mycompute_advantages(
        self,
        #values: torch.FloatTensor,
        logprop: torch.FloatTensor, 
        rewards: torch.FloatTensor,
        mask: torch.FloatTensor,
    ):
        lastgaelam = 0
        ...
        for t in reversed(range(gen_len)):
            if t < gen_len - 1:
                values[:, t] = rewards[:, t] - 0.06* logprop[:, t] + self.config.gamma* values[:, t + 1]
            else: 
                values[:, t] = rewards[:, t] - 0.06* logprop[:, t]
    ...
    def _save_pretrained(self, save_directory: str):
        ...
        #torch.save(self.accelerator.unwrap_model(self.model).deltaQ_head, os.path.join(save_directory, "q_head.pt"))
    ...
    def test_value(
        ...
        with torch.no_grad():
            ...
            values, advantages, returns = self.compute_advantages(values, rewards, masks)
            Myvalues = self.Mycompute_advantages(all_logprobs, rewards, masks)
        ...
        batch_dict = {
            ...
            "advantages": advantages,
            "returns": returns,
            "Myvalues": Myvalues.to(torch.float32),
        }
        batch_dict.update(model_inputs)
            ...
            for backward_batch_start in range(0, bs, self.config.backward_batch_size):
                ...
                for mini_batch_start in range(0, self.config.backward_batch_size, self.config.mini_batch_size):
                    ...
                    with self.accelerator.accumulate(self.model):
                        ...
                        train_stats = self.train_value_minibatch(
                            mini_batch_dict["values"],
                            vpreds,
                            mini_batch_dict["masks"],
                            mini_batch_dict["returns"],
    ...
    def train_value_minibatch(
        ...
        self.model.train()
        #self.accelerator.unwrap_model(self.model).set_all_adapter_trainable(False)
        loss_v, train_stats = self.value_loss(
            values, vpreds, mask, returns
        )
        self.accelerator.backward(loss_v)
        if self.config.max_grad_norm is not None:
            if self.accelerator.sync_gradients:
                self.accelerator.clip_grad_norm_(self.model_params, self.config.max_grad_norm)
        self.optimizer.step()
    ...
    def value_loss(
        ...
        return self.config.vf_coef * vf_loss, flatten_dict(stats)
