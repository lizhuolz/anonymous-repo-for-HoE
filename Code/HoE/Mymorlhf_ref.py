...
import importlib
from trl.trainer.My_ppo_trainer2 import MyPPOTrainer2
...
class ScriptArguments:
    ...
    target: Optional[float] = field(default=2, metadata={"help": "target kl divergence of adaptive control"})
...
humor_v_head = "yourpath/HoE/workspace/MOD/experiment-PPO/ppo/logs_Mymorlhf_train5/train1/epoch_1_batch_28/v_head1.pt"
...
moemodel = ConditionedMOEModelWithValueHead(model=model, v_heads_init=[humor_v_head, harmless_v_head], trainable_module=["lora"])
...
import copy
ref_moemodel = copy.deepcopy(moemodel)
ref_moemodel.train(False)
moemodel.train()
...
ppo_trainer = MyPPOTrainer2(
    config, moemodel, ref_model=ref_moemodel, tokenizer=tokenizer, dataset=dataset, data_collator=collator, optimizer=optimizer
)
...
for epoch in range(epochs):
    ...
    for i, batch in enumerate(ppo_trainer.dataloader):
        ...
        #ppo_trainer.log_stats(stats, batch, rewards)  #        bug，   ValueError: autodetected range of [nan, nan] is not finite
