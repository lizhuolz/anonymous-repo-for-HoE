...
class ScriptArguments:
    ...
    adapter_name: Optional[str] = field(default=None, metadata={'help':"the path to the sft model; need to merge if using lora"})
...
if reward_name == 'summary':
    ...
    reward_peft_path = ' yourpath/model/gpt2-large-helpful-reward_model'
elif reward_name == 'harmless':
    #reward_peft_path = 'Ray2333/gpt2-large-harmless-reward_model'
    reward_peft_path = ' yourpath/model/gpt2-large-harmless-reward_model'
    ...
    reward_peft_path = ' yourpath/model/humor-no-humor'
else:
...
for epoch in range(epochs):
    ...
    for i, batch in enumerate(ppo_trainer.dataloader):
        ...
        with torch.no_grad():
            response_tensors = ppo_trainer.generate(query_tensors, batch_size=32, return_prompt=False, **generation_kwargs) 
        ...
        # save model
        if ppo_trainer.accelerator.is_main_process and i % 10 == 0 and i != 0:
