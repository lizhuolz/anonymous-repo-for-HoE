...
from utils import print_trainable_parameters, load_main_tokenizer, Instructions, Instructions_summary, \
                  build_dataset, build_dataset_summary, build_dataset_beaver          
...
class ScriptArguments:
    ...
    batch_size: Optional[int] = field(default=60, metadata={"help": "the batch size64"})
    ...
    target: Optional[float] = field(default=6, metadata={"help": "target kl divergence of adaptive control"})
...
reward_model = RewardModels(reward_model_path_list, rm_tokenizer_path_list, gpu_id, " yourpath/workspace/RiC/RiC/ppo/logs_morlhf/reward_stats_defa.npy")
...
if exp_type == 'assistant':
    ...
    dataset = load_from_disk(" yourpath/dataset/assistant")
...
for epoch in range(epochs):
    ...
    for i, batch in enumerate(ppo_trainer.dataloader):
        ...
        with torch.no_grad():
            response_tensors = ppo_trainer.generate(query_tensors, return_prompt=False, batch_size=20, **generation_kwargs)
        ...
        # save model
        if ppo_trainer.accelerator.is_main_process and i % 70 == 0 and i != 0:
