...
import os 
def build_IF_dataset(path, tokenizer, rm_tokenizer, split='train', size=None):
    ds = load_from_disk(path, keep_in_memory=True)
    
    def map_template(example):
        ...
        example["input_ids"] = tokenizer.encode(example["prompt"])
        return example
    ds_concat = ds.map(map_template, batched=False, num_proc=30)
    ds_concat = ds_concat.remove_columns(['dataset', 'response'])
...
def build_dataset_beaver(path, tokenizer, rm_tokenizer, split='train', size=None):
    ds = load_dataset(path, split='train')
    if size is not None:
        ds = ds.select(range(size))
    if split=='test':
        ds = ds.select(range(0, len(ds), 12))
    if isinstance(rm_tokenizer,list):
        rm_tokenizer = rm_tokenizer[0]
    def tokenize(sample):
        sample['prompt'] = '\n\nHuman:' + sample['prompt'] + ' ' + '\n\nAssistant:'
        sample["input_ids"] = tokenizer.encode(sample["prompt"])
        sample["query"] = tokenizer.decode(sample["input_ids"])
        sample['reward_ids'] = rm_tokenizer.encode(sample['prompt']) # for data filter
        return sample

    ds_concat = ds.map(tokenize, batched=False, num_proc=30)
    ds_concat = ds_concat.filter(lambda x: len(x["input_ids"]) <= 256 and len(x["input_ids"]) >= 8 and len(x['reward_ids']) <= 256 and len(x['reward_ids']) >= 8)
    ds_concat = ds_concat.remove_columns(['response_0', 'response_1', 'is_response_0_safe', 'is_response_1_safe', 'better_response_id', 'safer_response_id'])

    ds_concat.set_format(type="torch")
    return ds_concat

"""
def build_dataset_steer(path, tokenizer, rm_tokenizer, split='train', size=None):
    if split == 'test':
        split = 'validation'
    ds = load_dataset(path, split=split)
    df = pd.DataFrame(ds)
    ds = Dataset.from_pandas(df.drop_duplicates("prompt"))
    ...
    def convert_to_chat_template(input_string):
        #    user   assistant      
        pattern = r"\n\n(user|assistant): (.+?)(?=\n\n(user|assistant):|$)"
    ...
    def tokenize(sample):
        ...
        sample['prompt'] = '\n\nHuman: ' + sample['prompt'] + ' ' + '\n\nAssistant: '
        sample['prompt'] = sample['prompt'].replace("<extra_id_1>Assistant\n", "\n\nAssistant: ")
        sample['prompt'] = sample['prompt'].replace("<extra_id_1>User\n", "\n\nHuman: ")

        sample['prompt'] = sample['prompt'].replace("\n\nAssistant: ", "\n\nassistant: ")
        sample['prompt'] = sample['prompt'].replace("\n\nHuman: ", "\n\nuser: ")
        #print(convert_to_chat_template(sample['prompt']))
        template_prompt = convert_to_chat_template(sample['prompt'])
        sample["input_ids"] = tokenizer.apply_chat_template(template_prompt)
        #sample["input_ids"] = tokenizer.encode(sample["prompt"])
        sample["query"] = tokenizer.decode(sample["input_ids"])
        sample['reward_ids'] = rm_tokenizer.encode(sample['prompt']) # for data filter
        return sample
    ...
    ds_concat = ds.map(tokenize, batched=False, num_proc=30)
    ds_concat = ds_concat.filter(lambda x: len(x["input_ids"]) <= 2048 and len(x["input_ids"]) >= 8 and len(x['reward_ids']) <= 2048 and len(x['reward_ids']) >= 8)
    ds_concat = ds_concat.remove_columns(['response', 'helpfulness', 'correctness', 'coherence', 'complexity', 'verbosity'])
...
def build_dataset_Psoups(path, tokenizer, rm_tokenizer, split='test', size=None):
    if split != 'test':
        raise NotImplementedError
    df = pd.read_json(" yourpath/dataset/Psoups_data/koala_eval_50.json")
    ...
    ds_concat = ds.map(tokenize, batched=False, num_proc=30)
    ds_concat = ds_concat.filter(lambda x: len(x["input_ids"]) <= 2048 and len(x["input_ids"]) >= 8 and len(x['reward_ids']) <= 2048 and len(x['reward_ids']) >= 8)
    #ds_concat = ds_concat.remove_columns(['id', 'helpfulness', 'correctness', 'coherence', 'complexity', 'verbosity'])
...
import sys
sys.path.append(" yourpath/workspace/safe-rlhf/")
from safe_rlhf.models import AutoModelForScore

def load_reward_model(reward_peft_path, gpu_id):
    if 'Armo' in reward_peft_path:  ### for Armo
    ...
    if "steer" in reward_peft_path:
        reward_model = AutoModelForSequenceClassification.from_pretrained(
            reward_peft_path, trust_remote_code=True,
    ...
    if "beaver" in reward_peft_path:
        ...
        return reward_model.to(gpu_id)
    num_labels = 2 if ('humor' in reward_peft_path or 'faithful' in reward_peft_path) else 10 if ('steer' in reward_peft_path) else 1
    reward_model = AutoModelForSequenceClassification.from_pretrained(
                    reward_peft_path,
...
def get_rewards(reward_model, texts_for_rewards, reward_mean_std=None, sub_position=-1):
    rewards = []
    print('log: reward model forwarding ...')
    with torch.no_grad():
        pbar = tqdm(total=len(texts_for_rewards))
        for inputs in texts_for_rewards:
            if sub_position == -100: # for beaver reward model output
                rewards.append(reward_model(**(inputs.to(reward_model.device))).end_scores[0])
            if sub_position == "Armo": ###for Armo
                score = reward_model(inputs.to(reward_model.device)).score.float().item()
                rewards.append(torch.tensor(score))
            elif sub_position != -1: # for multiple output   # maybe for  steer output ?
