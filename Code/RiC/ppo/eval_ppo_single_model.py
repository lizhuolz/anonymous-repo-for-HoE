...
from utils import Instructions, Instructions_summary, build_dataset_eval, build_dataset_summary_eval, build_dataset_beaver, build_dataset_steer, build_dataset_Psoups, check_lora_in_model_path, merge_lora_weight
tqdm.pandas()
...
steer2_dataset_path = 'yourpath/HoE/dataset/HelpSteer2'
steer_dataset_path = "yourpath/HoE/dataset/HelpSteer"

@dataclass
class ScriptArguments:
    log_with: Optional[str] = field(default='wandb', metadata={"help": "use 'wandb' to log with wandb"})
    save_directory: Optional[str] = field(default='./logs_ppo_assistant')
    batch_size: Optional[int] = field(default=20, metadata={"help": "the batch size"})
    ...
    tokenizer_name: Optional[str] = field(default='yourpath/HoE/model/assistant_sft_llama')
...
reward_path_tokenizer_dict = {
    ...
    'steer_5reward': ["yourpath/HoE/model/steer_rm/RewardModel-Mistral-7B-for-DPA-v1"],
}
...
def get_clean_data(full_responses, full_prompts):
    ...
    for i, response in enumerate(full_responses):
        ...
        full_prompts[i] = full_prompts[i].strip().replace('[PAD] ', '').replace('[PAD]', '')
        #response = response.strip('[PAD] ')
        response = response.strip().replace('[PAD] ', '').replace('[PAD]', '')
        #temp_resp = response.replace(full_prompts[i], '').strip('<s>').strip('</s>')
        temp_resp = response.replace(full_prompts[i], '').replace('<s>', '').replace('</s>', '')
        if '</s>' in temp_resp:
        ...
        temp_resp = temp_resp.split('\nAssistant:')[0].strip()
        temp_resp = temp_resp.split('\n\n\n')[0].strip()
        #full_responses_clean.append(full_prompts[i].replace("\n\nuser: ", '\n\nHuman: ') + ' ' + temp_resp)
...
if exp_type == 'assistant':
    ...
    valid_dataset = build_dataset_beaver(beaver_dataset_path, tokenizer, reward_models.rm_tokenizers,  split='test')
    instructions = Instructions()
elif exp_type == 'steer2':
    valid_dataset = build_dataset_steer(steer2_dataset_path, tokenizer, reward_models.rm_tokenizers,  split='test')
    instructions = Instructions()
elif exp_type == 'steer':
    valid_dataset = build_dataset_steer(steer_dataset_path, tokenizer, reward_models.rm_tokenizers,  split='test')
    instructions = Instructions()
elif exp_type == 'psoups':
    valid_dataset = build_dataset_Psoups(None, tokenizer, reward_models.rm_tokenizers,  split='test')
    instructions = Instructions()
else:
...
valid_batch_size = int(script_args.batch_size)
#valid_dataset = valid_dataset.remove_columns(['prompt', 'query'])
for key in ['key', 'text', 'response', 'prompt', 'query', 'reward_ids']:
...
all_rewards = []
num_rewards = reward_models.num_rewards+4 if 'steer_5reward' in reward_names else reward_models.num_rewards
