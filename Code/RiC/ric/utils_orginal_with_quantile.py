...
def get_rewards(reward_model, texts_for_rewards, reward_mean_std=None, sub_position=0):
    ...
    with torch.no_grad():
        ...
        for inputs in texts_for_rewards:
            ...
            elif sub_position != 0: # for multiple output
...
def sample_goals(size, num_rewards=2, rewards_list=None, maximum=0.9999):
    if rewards_list is None:
        samples = np.random.normal(0, 1, 100000) * 0.5 + 0.5
...
def select_data_with_quantile(dataset, quantile_threshold, weighting=None, num_scores=None):
    if num_scores == None:
        num_scores = 0
...
def My_select_data_with_quantile(dataset, quantile_threshold, weighting, num_scores=None):
    if num_scores == None:
        num_scores = 0
        for key in dataset.column_names:
            if key.startswith('score'):
                num_scores += 1
    score_columns = ['score{}'.format(i+1)  for i in range(num_scores)]
    assert len(score_columns) == len(weighting)
    def compute_weighted_score(example):
        weighted_sum = sum(example[col] * w for col, w in zip(score_columns, weighting))
        return {"weighted_score": weighted_sum}
    dataset = dataset.map(compute_weighted_score)

    threshold = np.quantile(dataset['weighted_score'], quantile_threshold)
    dataset = dataset.filter(lambda x: x["weighted_score"] >= threshold)
    dataset = dataset.remove_columns(["weighted_score"])
    return dataset

def dataset_from_csv_n(checkpoint_path, tokenizer, weighting, exp_type='assistant', quantile_threshold=0.7):
    generated_dataset = pd.read_csv(checkpoint_path + '/data.csv')
    generated_dataset = Dataset.from_pandas(generated_dataset)
    ...
    # remove data
    generated_dataset = select_data_with_quantile(generated_dataset, quantile_threshold, weighting, num_scores)
    generated_dataset = generated_dataset.map(process, batched=False, num_proc=20)
    return generated_dataset


def merge_dataset(dataset, online_dataset, save_path, tokenizer_name, weighting, info_path=None, sample_origin=10000, exp_type='assistant', quantile_threshold=0.7):
    tokenizer = load_main_tokenizer(tokenizer_name)
    if type(dataset) == str:
        dataset = load_from_disk(dataset)

    ## select frontier dataset
    dataset = select_data_with_quantile(dataset, quantile_threshold, weighting)
    ...
    generated_dataset = dataset_from_csv_n(save_path, tokenizer, weighting, exp_type=exp_type, quantile_threshold=quantile_threshold)
