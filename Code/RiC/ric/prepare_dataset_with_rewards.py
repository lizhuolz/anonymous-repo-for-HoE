...
tokenizer_name = 'yourpath/model/llama-2-7b-hf'
...
def build_dataset_beaver(index, tokenizer, rm_tokenizers, split='train'):
    ds = load_dataset(beaver_dataset_path, split=split)
    n = len(rm_tokenizers)
    ...
    def tokenize(sample, reject=False):
        ...
        if not reject:
            ...
            sample['response'] = sample['response_1'] 
...
def build_dataset_summary(index, tokenizer, rm_tokenizers, split='train'):
    ...
    ds = load_from_disk(summary_dataset_path)
    ds = ds[split]
...
if Accelerator().num_processes == 1:
    ...
    add_score = add_score_summary if script_args.exp_type == 'summary' else add_score_assistant
