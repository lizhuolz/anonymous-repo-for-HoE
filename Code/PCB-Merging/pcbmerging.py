...
class ScriptArguments:
    sft_model_path: Optional[str] = field(default='wandb')
    model0_path: Optional[str] = field(default="./eval_svd64")
    model1_path: Optional[str] = field(default="./eval_svd64")
    weighting: Optional[float] = field(default=1.0)
    save_path: Optional[str] = field(default="./eval_svd64")
    
parser = HfArgumentParser(ScriptArguments)
script_args = parser.parse_args_into_dataclasses()[0]
sft_model_path = script_args.sft_model_path
model0_path = script_args.model0_path
model1_path = script_args.model1_path
save_path = script_args.save_path
weightings = [float(script_args.weighting), 2.0 - float(script_args.weighting)]
...
with torch.no_grad():
    ...
    flat_tvs  =  torch.stack([peft_vector0, peft_vector1], dim=0)
    print(flat_tvs.shape)
    del peft_vector0, peft_vector1, peft_dict1
    ...
    #fusion_peft_model.save_pretrained(" yourpath/workspace/pcbmerging/help0.95harm1.05_pcbmerging")
    # yourpath/workspace/pcbmerging/3sft_helphumor_pcbmerging
    fusion_peft_model.save_pretrained(save_path)
